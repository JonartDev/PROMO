...
function b_submit_csv() {
    ...
    $.ajax({
        ...
        url: "/promo3betlist/save_data_csv",
        data: data,
        processData: false,
        contentType: false,
        cache: false,
    ...
    $.confirm({
        ...
        buttons: {
            confirm: {
                ...
                text: '删除/ Delete Data',
                action: function(){
                    $.get("/promo3betlist/delete_unprocessed",
...
function b_upgrade()
{
    $.confirm({
        ...
        icon: 'bi-exclamation-circle',
        content: '确认将要更新用户信息和活动3的数据 / You are about to update userinfo and promo3 data',
        autoClose: 'cancelAction|10000',
        type: 'orange',
        escapeKey: 'cancelAction',
        buttons: {
            confirm: {
                ...
                action: function(){
                    spinner();
                    $.get("/promo3betlist/process_data",
                    function(data, status){
                        $('#promo3_datatable').DataTable().ajax.reload();
                        $("#toast_msg").html(data.toast);
                        toast1.show();
