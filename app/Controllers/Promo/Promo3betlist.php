 ...
 * 
 */
class Promo3betlist extends BaseController
{
    /**
     * 
     */
    ...
    public function index()
    {
        $dataset = [
            ...
            'js' => ['/assets/bootstrap-datepicker/js/bootstrap-datepicker.js', '/assets/js/promo3betlist.js'] // use to add js specific to page.
        ];

        return view('admin/v_header', $dataset)
            . view('promo/v_promo3betlist')
            . view('admin/v_footer');
    }
    ...
    public function datatable()
    {
        ...
        $Betlist3Model = model(Betlist3Model::class);

        $orderby = $postdata['columns'][$postdata['order'][0]['column']]['data']; // get proper orderby string

        $dataset = $Betlist3Model->get_datatable_data($postdata['length'], $postdata['start'], $orderby, $postdata['order'][0]['dir'], $postdata['search']['value']);
    ...
    public function save_data_csv()
    {
        ...
        foreach ($dataset as $row) {
            $insert_data = $Betlist3Model->insert_data($row);
            $new += 1;
        }
    ...
    {
        $Betlist3Model = model(Betlist3Model::class);
        $query = $Betlist3Model->delete_data_params(['status' => 0]);
    ...
    public function process_data()
    {
        ...
        $dataset = $Betlist3Model->get_distinct_user_params(['status' => 0]);
        $user_insert = 0;
        $user_update = 0;
	$promo_insert = 0;
	$groupedPrizes = [];
        foreach ($dataset as $row) {
            $rowset = $Betlist3Model->get_distinct_sum_params(['username' => $row['username']]);
            $rowset = $rowset[0];
            //USERINFO. insert IF not exist, edit if exist.
            $checkuserinfo = $Userinfo3Model->get_data_params(['username' => $row['username']]);
            ...
            if (count($checkuserinfo)) {
                ...
                $updateuserinfo = $Userinfo3Model->update_data_params(['username' => $row['username']], $payload);
                $user_update += 1;
            } else {
                //insert
                $payload = [
                ...
                $insertuserinfo = $Userinfo3Model->insert_data($payload);
                $user_insert += 1;
            }
            ...
            if($vip_level != 0)
            {
                for($i = 1; $i <= $vip_level; $i++)
                {
                    ...
                    $checkpromo = $Promo3Model->get_data_params( ['username' => $row['username'], 'vip_level' => $i] );
                    if( count($checkpromo) == 0 )
                    {
                        ...
                        $insertpromo = $Promo3Model->insert_data( $payload );
                        $promo_insert += 1;
                    }
                }
            }          
        }
        $update_betlist = $Betlist3Model->update_data_params(['status' => 0], ['status' => 1]);
