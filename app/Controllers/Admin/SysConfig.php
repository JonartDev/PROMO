    ...
    public function save_data()
	{
		...
		if (!empty($_FILES['photo']['name'])) {
		  if ($photo->isValid() && !$photo->hasMoved()) {
		    ...
		    $destinationPath = FCPATH . 'sysconfig_assets/uploads/' . $newName;
		    // Move the uploaded file to the desired destination
		    $photo->move(FCPATH . 'sysconfig_assets/uploads/', $newName);
		    // Update the sys_value in $postdata
		    $postdata['sys_value'] = 'sysconfig_assets/uploads/' . $newName;
		  }
		}
		...
		if($postdata['action_type'] == 'add')
		{
			...
			return $this->response->setJSON(['status' => 0, 'validation' => 'An error occurred in the response.']);
	      }
