 ...
 */
class Betlist8167Model extends Model
{
    protected $table      = 'betlist_8167';
    ...
    protected $allowedFields = ['username', 'bet', 'bet_date', 'status'];
    ...
    public function get_datatable_data(int $limit = 0, int $offset = 0, string $orderby = null, string $orderdir = 'ASC', string $search = null )
    {
        ...
        $dataset->select('id, username, bet, bet_date, status, created_at, updated_at');
        ...
        if($search != null)
        {
            ...
            $dataset->orLike('bet', $search);
            $dataset->orLike('bet_date', $search);
    ...
    public function get_distinct_sum($date)
    {
        $dataset = $this;
        $dataset->select('username, sum(bet) as total');
        $dataset->where('bet_date', $date);
        $dataset->groupBy('username');

        return $dataset->findAll();
    }

    public function get_distinct_user_params($data)
    {
        $dataset = $this;
        $dataset->select('username');
        foreach( $data as $key => $value )
        {
            $dataset->where($key, $value);
        }
    ...
    public function get_distinct_sum_params($data)
    {
        $dataset = $this;
        $dataset->select('id, username, sum(bet) as total, max(bet_date) as bet_date, status');
        foreach( $data as $key => $value )
        {
            $dataset->where($key, $value);
        }
    ...
    public function get_latest_record(array $data = [])
    {
        ...
        $dataset->orderBy('id', 'DESC'); 
        $dataset->limit(1);
        $result = $dataset->findAll();
